...
def process_file(file):
	...
	sleeper, minute = most_probable_sleeper(ocurrences)
	return sleeper * minute

def most_probable_sleeper(ocurrences):
	maximum = 0
	p_sleeper = None
	p_minute = None
	for k,v in ocurrences.items():
		result, minute = build_matrix(ocurrences[k])
		if(result > maximum):
			p_sleeper = k
			maximum = result
			p_minute = minute 
	return p_sleeper, p_minute

def build_matrix(ocurrences):
	matrix = np.zeros((50,))
	for i in range(0, len(ocurrences)-1, 2):
		matrix[ocurrences[i]:ocurrences[i+1]] += 1
	return np.max(matrix), np.argmax(matrix)
